...
Same as `%header`, `%source` and `%common`, respectively. The only difference is that the early directives place the code at the very beginning of the generated file, before any code or includes generated by PackCC. This can be usefull for example when it's necessary to modify behavior of standard libraries via a macro definition.
